...
class ImageAcquisitionThread(threading.Thread):
    ...
    def generate_video(self):
        image_folder = '.'
        video_name = 'result2.mp4' #file name. 
        os.chdir(".../ThorCam")
        # Change above path to where you want to video to be saved
        pil_images = list(self._image_queue.queue)
        images = [numpy.asarray(i) for i in pil_images]
...
if __name__ == "__main__":
    with TLCameraSDK() as sdk:
        ...
        with sdk.open_camera(camera_list[0]) as camera:     #Open the first camera available

            # create generic Tk App with just a LiveViewCanvas widget
            print("Generating app...")
            root = tk.Tk()
            root.title(camera.name)
            image_acquisition_thread = ImageAcquisitionThread(camera)       #Aquire the frame images from camera and put in thread
            camera_widget = LiveViewCanvas(parent=root, image_queue=image_acquisition_thread.get_output_queue())        # Display the camera video
            ...
            print("App starting")
            root.mainloop()
            image_acquisition_thread.generate_video()
